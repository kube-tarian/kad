...
func updateCreds(clustername string, creds map[string]string) bool {
	...
	if cluster, ok := managedClusterData[clustername]; ok {
		...
		// kubeConfig is not included in the check, because everytime token is getting updated
		if cluster.Creds[k8sClusterCA] != creds[k8sClusterCA] || cluster.Creds[k8sEndpoint] != creds[k8sEndpoint] {
