...
func (c *Cassandra) Lock() error {
	...
	return database.CasRestoreOnErr(&c.isLocked, false, true, database.ErrLocked, func() error {
		...
		const startSchemaMigrationCQL = `INSERT INTO schema_change.lock (keyspace_to_lock, started_at) VALUES (?,toTimestamp(now())) IF NOT EXISTS USING TTL ?`
		lockAcquired, err := c.session.Query(startSchemaMigrationCQL, c.config.KeyspaceName, LockTTLSeconds).ScanCAS(&keyspace, &startedAt)
		if err != nil {
			return err
		}
		if !lockAcquired {
...
func (c *Cassandra) Unlock() error {
	...
	return database.CasRestoreOnErr(&c.isLocked, true, false, database.ErrNotLocked, func() error {
		const finishSchemaMigrationCQL = `DELETE FROM schema_change.lock WHERE keyspace_to_lock = ? IF EXISTS`
		return c.session.Query(finishSchemaMigrationCQL, c.config.KeyspaceName).Exec()
	})
}
